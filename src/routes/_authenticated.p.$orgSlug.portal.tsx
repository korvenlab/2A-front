...
export const Route = createFileRoute("/_authenticated/p/$orgSlug/portal")({
...
function Portal() {
  ...
  const routeSlugDecoded = decodeURIComponent(orgSlug);
  ...
  const load = async () => {
    ...
    const byUrlSlug = orgList.find((o) => o.slug && o.slug === routeSlugDecoded);
    let chosenOrgId: string | null = null;
    if (byUrlSlug && linkedIdSet.has(byUrlSlug.id)) {
      chosenOrgId = byUrlSlug.id;
    } else if (inviteOrgId && linkedIdSet.has(inviteOrgId)) {
    ...
    const canonicalRow = chosenOrgId ? orgList.find((o) => o.id === chosenOrgId) : null;
    const canonicalSlug = canonicalRow?.slug?.trim() ?? "";
    if (chosenOrgId && !canonicalSlug) {
      navigate({ to: "/portal", search: searchRecordFromWindow(), replace: true });
      setLoading(false);
      return;
    }
    if (chosenOrgId && canonicalSlug && canonicalSlug !== routeSlugDecoded) {
      navigate({
        to: "/p/$orgSlug/portal",
        params: { orgSlug: canonicalSlug },
        search: searchRecordFromWindow(),
        replace: true,
      });
      setLoading(false);
      return;
    ...
    if (chosenOrgId && canonicalSlug) {
      persistPortalOrgContext(chosenOrgId, canonicalSlug);
    }
  ...
  const onPortalOrgChange = (nextId: string) => {
    const row = linkedOrgs.find((o) => o.id === nextId);
    ...
    if (s) {
      navigate({ to: "/p/$orgSlug/portal", params: { orgSlug: s }, replace: true });
    }
  };
  ...
  }, [user?.id, organization?.id, inviteToken, pinnedOrgId, routeSlugDecoded]);
